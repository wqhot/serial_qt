...
class SerialReaderThread(threading.Thread):
    ...
    def run(self):
        while self.running:
            if (
                self.serial_port.read() == b"\xeb"
                and self.serial_port.read() == b"\x90"
            ):
...
class MainWindow(QWidget):
    ...
    def update_line_edits(self, data: SerialData):
        self.line_edit_widgets[0].setText(str(data.utime))
        for i, value in enumerate(data.params):
            if i >= len(self.line_edit_widgets):
                return
            self.line_edit_widgets[i + 1].setText(str(value))
        if len(data.params) + 3 > len(data.params):
            return
